...
class SettlementService:
    def initiate(self, request: SettlementRequest) -> dict:
        ...
        if not offer:
            return SettlementResponse(status="OFFER_NOT_FOUND", amount_paise=0).model_dump()
        ...
        with db_session() as conn:
            conn.execute(
                ...
                "final_amount_paise=? WHERE id=?",
                (amount_paise, request.session_id),
            )
        offer_service.set_status(offer["id"], "SETTLEMENT_INITIATED")

        entry = audit.log(request.session_id, "SETTLEMENT_INITIATED", "system",
